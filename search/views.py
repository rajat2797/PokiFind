...
def tweet(request):
	text=request.GET.get('text') or ''
	s='https://twitter.com/home?status='
...
def search_db(search_string=''):
	...
	for pokemon in Pokedex.objects.all():
		if pokemon.lower().startswith( search_string.lower() ):
...
def short_url(request,pokemon_id):
	try:
		...
		return redirect('/pokemon/156/Wigglytuff')
	return redirect('/pokemon/%s/%s'%(pokemon_id,pokemon.pokemon_name))

def desc(request,pokemon_id,pokemon_name):
	...
	return render(request,'search/desc.html',context_dict)
...
def game(request):
	...
	random_pokemon.append(correct_pokemon)
	for i in range(4):
